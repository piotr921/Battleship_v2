...
public class SocketClient {
    ...
    void setUpStreamsOnSocket() throws IOException {
        logger.log(AppLogger.DEFAULT_LEVEL, "Setting up streams and socket");
    ...
    public void createUpdatesThreadAndRunIt() {
        logger.log(AppLogger.DEFAULT_LEVEL, "Creating a thread for reading server updates");
        updatesThread = new Thread(this::readServerUpdatesContinuously);
        updatesThread.start();
    }

    private void readServerUpdatesContinuously() {
        logger.log(AppLogger.DEFAULT_LEVEL, "Reading server updates continously");
        while (isRunning) {
            try {
                this.responsesBus.addAServerResponse(readUpdateFromServer());
                Thread.sleep(100);
            } catch (IOException | InterruptedException e) {
                logger.log(AppLogger.DEFAULT_LEVEL, "An exception while reading server responses: ", e);
                break;
            }
        }
    }
    ...
    public void closeTheSocketClient() {
        try {
            ...
            logger.log(AppLogger.DEFAULT_LEVEL, "An exception while closing the SocketClient: ", e);
        }
    }

    public void sendStringToServer(String string) {
        try {
            logger.log(AppLogger.DEFAULT_LEVEL, "Attempting sent string to server = " + string);
            this.output.println(string);
            logger.log(AppLogger.DEFAULT_LEVEL, "Sent string to server = " + string);
